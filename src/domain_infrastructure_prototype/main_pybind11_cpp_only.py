...
#    sum_weight = np.zeros((n_imag_chan, n_imag_pol), dtype=np.double)
    ...
    print(np.sum(grid))
    #grid = np.full((n_chan_chunks*chan_chunk_size, n_imag_pol, image_size, image_size), np.complex128(0.0),dtype=np.complex128) #super slow

    grid_shape = np.array(grid.shape)
    sum_weight = np.zeros((n_chan_chunks*chan_chunk_size, n_imag_pol), dtype=np.double)
    
    print('grid_size ',np.prod(grid_shape) ,',*,',grid_shape)
    ...
    for i_time_chunk in range(n_time_chunks):
        for i_chan_chunk in range(n_chan_chunks):
            ...
            gridder.grid_vis_data(grid_shape,sum_weight,vis_data,vis_shape,uvw, freq_chan, chan_map, pol_map, weight, cgk_1D, delta_lm, support, oversampling)
