...
def _standard_grid_jit(grid, grid_shape, sum_weight, vis_data, vis_shape, uvw, freq_chan, chan_map, pol_map, weight, cgk_1D, delta_lm, support, oversampling):
    ...
    for i_time in range(n_time):
        for i_baseline in range(n_baseline):
            for i_chan in range(n_chan):
                ...
                u = uvw[i_time, i_baseline, 0] * (-(freq_chan[i_chan] * delta_lm[0] * n_u) / c)
                v = uvw[i_time, i_baseline, 1] * (-(freq_chan[i_chan] * delta_lm[1] * n_v) / c)
...
#
#
#def _standard_grid_jit(grid, grid_shape, sum_weight, do_psf, do_imaging_weight, vis_data, vis_shape, uvw, freq_chan, chan_map, pol_map, weight, cgk_1D, delta_lm, support, oversampling):
#
#    n_time = vis_shape[0]
#    n_baseline = vis_shape[1]
#    n_chan = vis_shape[2]
